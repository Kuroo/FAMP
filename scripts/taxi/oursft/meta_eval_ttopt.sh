...
for SEED in "${SEEDS[@]}"; do
    RUN_DATA_DIR="${EXPERIMENTS_DIR}/taxi_oursft/eval/seed${SEED}"
    CHP="${EXPERIMENTS_DIR}/taxi_oursft/pretrain/seed${SEED}/checkpoints/epoch2000.tar"
    for ENV in "${ENVS[@]}"; do
        RUN_NAME="env${ENV}"
        ...
        --policy_type $POLICY_TYPE --chp_freq $CHP_FREQ --fixed_env $ENV --load_chp $CHP --term_time $TERM_TIME --save_trajs --no_bias
