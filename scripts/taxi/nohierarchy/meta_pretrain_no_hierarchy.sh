...
for SEED in "${SEEDS[@]}"; do
    RUN_NAME="seed${SEED}"
    LOG_DIR="${EXPERIMENTS_DIR}/taxi_no_hierarchy/${RUN_NAME}/logs/"
    conda run -n famp mpiexec -np $NCORES -prepend-rank -outfile-pattern "${LOG_DIR}stdout.txt" -errfile-pattern "${LOG_DIR}stderr.txt" \
    python -u main.py --baseline $BASELINE --seed "$SEED" --env $ENV_NAME --run_name "$RUN_NAME" --run_data_dir "${EXPERIMENTS_DIR}/taxi_no_hierarchy"  --epochs $EPOCHS --envs_per_process $ENVS_PER_PROCESS \
    ...
    --exclude_envs ${EXCLUDE_ENVS[*]} --return_discount $RET_DISC --policy_type $POLICY_TYPE --no_bias --adapt_options --termination_prior 0.5
done
