...
for SEED in "${SEEDS[@]}"; do
    RUN_DATA_DIR="${EXPERIMENTS_DIR}/ant_maze_famp/eval/seed${SEED}"
    CHP="${EXPERIMENTS_DIR}/ant_maze_famp/pretrain/seed${SEED}/checkpoints/epoch02500.tar"
    for ENV in "${ENVS[@]}"; do
        RUN_NAME="env${ENV}"
        LOG_DIR="${RUN_DATA_DIR}/ant_maze_famp/eval/seed${SEED}/logs/"
        ...
        --hidden_sizes_subpolicy ${HID_SUBP[*]} --chp_freq $CHP_FREQ --learn_lr_inner --fixed_env $ENV --load_chp $CHP --save_trajs
    done
done
